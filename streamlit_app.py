...
def fallback_predict(features: np.ndarray) -> float:
    # Simple deterministic fallback when model file is not available.
...
def main():
    ...
    if model is not None:
        ...
        st.warning(
            "Aucun modele trouve (modele/modele_pokemon.mod). "
            "L'application utilise une estimation de secours."
        )
    ...
    if st.button("Predire", type="primary"):
        ...
        with st.expander("Details de l'entree"):
            st.json(
                {
                    "hp": hp,
                    "attack": attack,
    ...
    st.caption(
        "Execution locale: streamlit run streamlit_app.py | "
        "CI GitHub Actions: workflow streamlit-ci.yml"
    )
